...
const MovieType = new GraphQLObjectType({
    ...
    fields: () => ({
        ...
        director: {
            type: DirectorType,
            resolve(parent, args) {
                return directors[directors.findIndex(director => director.id === parent.id)]; // связать текущее поле с полем другой таблицы
            }
        }
    }),
})
...
const Query = new GraphQLObjectType({
    ...
    fields: {
        movie: {
            ...
            resolve(parent, args) {
                return movies[movies.findIndex(movies => movies.id === args.id)];
            },
        },
        director: {
            type: DirectorType,
